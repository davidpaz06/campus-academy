...
export function nextModule(
  ...
  if (activeModule === course.modules.length - 1) {
    createModule(setCourse, course, setActiveModule);
  } else {
    setActiveModule(activeModule + 1);
  }
}

export function createModule(setCourse: (course: any) => void, course: any, setActiveModule: (idx: number) => void) {
  const newModule = {
...
import type { Course, CourseLesson, CourseModule } from "@/types/createcourse";
